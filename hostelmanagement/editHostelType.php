...
if (strlen($_SESSION['adminsession']==0)) {
	...
	if(isset($_POST['add']))
  {
    $title=$_POST['title'];
    $id=$_POST['id'];
    $query=mysqli_query($con,"update hostel_type set hostelType = '$title' where hostelTypeId=$id");
    //echo $query."print";
		if($query){
		echo '<script>alert("Hostel Type Updated Successfully")</script>';
    }
    else{
     echo '<script>alert("Failed TO update Hostel Type.")</script>';
    }
  }
  
   $get=mysqli_query($con,"select * from hostel_type where hostelTypeId=$hId");
  foreach($get as $Data):
	$titleg=$Data['hostelType'];
	$idg=$Data['hostelTypeId']; 
	endforeach;
?>
<!DOCTYPE html>
<html lang="en">
...
<br>
					...
					<br>
					<input type="text"  class="name" name="title"  pattern="^[a-zA-Z ]+$" value="<?php echo $titleg;?>" title="Only letters and Space" placeholder="Title" required="">
					<br>
					<br>
					<input type="hidden"  class="name" name="id"   value="<?php echo $idg;?>"  placeholder="Title" required="">
					
					<input type="submit" name="add" value="Update Hostel Type">
				</form>
			</div>	
			</div>
			</div>		
