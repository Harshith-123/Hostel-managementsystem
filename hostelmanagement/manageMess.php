...
if (strlen($_SESSION['adminsession']==0)) {
	...
	if(isset($_POST['add']))
  {
    $name=$_POST['Name'];
    $fees=$_POST['fees'];
    $query=mysqli_query($con,"insert into mess_tbl(messName,messFee)values('$name','$fees')");
    //echo $query."print";
		if($query){
		echo '<script>alert("Mess details Added Successfully")</script>';
    }
    else{
     echo '<script>alert("Failed TO Add Mess details.")</script>';
    }
  }
?>
<!DOCTYPE html>
...
?>
    ...
    <td><?=$title;?></td>
    <td><?=$fee;?></td>
	<td><a href="editMess.php?edit=<?= $id ?>" class="text-danger">Edit</a></td>
   
	<td><a href="manageMess.php?delete=<?= $id ?>" class="text-danger">Delete</a></td>
	 </tr>
	<?php
	$count+=1;
	endforeach;
...
<br>
					...
					<div class="form-group">
                        <label class="form-label" id="nameLabel" for="name">Mess Name</label>
                        <input type="text" class="form-control" id="Name" name="Name" placeholder="Mess  name" tabindex="1">
                    </div>
					<div class="form-group">
                        <label class="form-label" id="nameLabel" for="name">Mess Fees</label>
                        <input type="text" class="form-control" id="fees" name="fees" placeholder="Mess Fees" tabindex="1">
                    </div>
					<br>
					<br>
					<input type="submit" name="add" value="Add Mess">
				</form>
			</div>	
			</div>
			</div>		
...
if(isset($_GET['delete'])){
    $del_id = $_GET['delete'];
     $update=mysqli_query($con,"delete from mess_tbl where messId='$del_id'" );
    if($update){
        redirect('manageMess');
    }
    else{
        alert('Unable to delete mess  details');
    }     
}
?>
