...
if (strlen($_SESSION['adminsession']==0)) {
  ...
  {
    $title=$_POST['title'];
    $query=mysqli_query($con,"insert into hostel_type (hostelType) values ('$title')");
    //echo $query."print";
		if($query){
		echo '<script>alert("Hostel Type Added Successfully")</script>';
    }
    else{
     echo '<script>alert("Failed TO Add Hostel Type.")</script>';
    }
  }
...
<body>
  ...
  foreach($get as $Data):
	...
	$id=$Data['hostelTypeId'];    
?>
    <tr>
      <th scope="row"><?=$count;?></th>
      <td><?=$title;?></td>
	  		<td><a href="editHostelType.php?edit=<?= $id ?>" class="text-danger">Edit</a></td>
    <td><a href="adminHome.php?delete=<?= $id ?>" class="text-danger">Delete</a></td>
...
<br>
<br>
					<label for="exampleFormControlSelect2">Enter Type</label>
					<br>
					<input type="text"  class="name" name="title"  pattern="^[a-zA-Z ]+$" title="Only letters and Space" placeholder="Title" required="">
					<br>
					<br>
					
					<input type="submit" name="add" value="Add Hostel Type">
				</form>
			</div>	
			</div>
			</div>		
