...
if (strlen($_SESSION['adminsession']==0)) {
	...
	if(isset($_POST['add']))
  {
    ...
    $fee=$_POST['fees'];
    $id=$_POST['id'];
    $query=mysqli_query($con,"update hostel_tbl set hostelName = '$title',hostelAddress='$address',hostelMobile='$mobile',hostelFee='$fee' where hostelId=$id");
    //echo $query."print";
		if($query){
		echo '<script>alert("Hostel Updated Successfully")</script>';
    }
    else{
     echo '<script>alert("Failed TO update Hostel .")</script>';
    }
  }
  
   $get=mysqli_query($con,"select * from hostel_tbl where hostelId=$hId");
  foreach($get as $Data):
	$hostelName=$Data['hostelName'];
	$hostelMobile=$Data['hostelMobile'];
...
<br>
					<div class="form-group">
                        ...
                        <input type="text" class="form-control" id="Name"  value="<?php echo $hostelName;?>" name="Name" placeholder="Hostel  name" tabindex="1">
                    </div>
					<div class="form-group">
                        <label class="form-label" id="nameLabel" for="name">Hostel Phone Number</label>
                        <input type="text" class="form-control" id="phone" value="<?php echo $hostelMobile;?>" name="phone" placeholder="Hostel Phone Number" tabindex="1">
                    </div>
					<div class="form-group">
                        <label class="form-label" id="nameLabel" for="name">Hostel Address</label>
                        <input type="text" class="form-control" value="<?php echo $hostelAddress;?>" id="address" name="address" placeholder="Hostel Address" tabindex="1">
                    </div>
					<div class="form-group">
                        <label class="form-label" id="nameLabel" for="name">Hostel Fees</label>
                        <input type="text" class="form-control"  value="<?php echo $hostelFee;?>"id="fees" name="fees" placeholder="Hostel Fees" tabindex="1">
                    </div>
					<input type="hidden"  class="name" name="id"   value="<?php echo $idg;?>"  placeholder="Title" required="">
					
					<input type="submit" name="add" value="Update Hostel ">
				</form>
			</div>	
			</div>
			</div>		
...
if(isset($_GET['delete'])){
    $del_id = $_GET['delete'];
     $update=mysqli_query($con,"delete from hostel_type where hostelTypeId='$del_id'" );
    if($update){
        redirect('adminHome');
    }
    else{
        alert('Unable to delete Hostel Type details');
    }     
}
?>
